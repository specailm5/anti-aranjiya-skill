...
│
├── scripts/
│   └── lint_aranjiya.py                  # أداة تدقيق آلي لفحص النصوص واستخراج مواضع العرنجية
│
├── tests/
│   ├── sample_aranjiya_text.txt          # نص تجريبي مليء بالشواهد العرنجية لاختبار المدقق
│   └── sample_classical_text.txt         # نص فصيح نقي لاختبار اجتياز الفحص النظيف
│
...
## 🧪 الاختبار والتدقيق الآلي (Testing & Linting)

تتضمن المهارة أداة سريعة مكتوبة بـ Python لاكتشاف مواضع العرنجية تلقائياً واقتراح البدائل الفصيحة، مع إمكانية دمجها في خطوط المعالجة الآلية (CI/CD) و Git Hooks:

### ١. فحص ملف نصي تجريبي:
```bash
python scripts/lint_aranjiya.py tests/sample_aranjiya_text.txt
```
ستعرض الأداة تقريراً مفصلاً يتضمن رقم السطر، اللفظ المرصود، العلة البلاغية والنحوية، والبديل الفصيح المباشر.

### ٢. اختبار اجتياز النص الفصيح:
```bash
python scripts/lint_aranjiya.py tests/sample_classical_text.txt
```
سيجتاز الفحص بنجاح بدون أي تنبيهات (`Exit Code: 0`).

### ٣. فحص نص مباشر عبر سطر الأوامر:
```bash
python scripts/lint_aranjiya.py --text "تم توقيع الاتفاقية بواسطة الوزير من خلال مؤتمر صحفي حيث أجرى محادثات مكثفة."
```
